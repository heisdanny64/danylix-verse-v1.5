...
export default function Player() {
  ...
  useEffect(() => {
    ...
    // Gifted proxy stream URLs are plain MP4-over-HTTP, not m3u8.
    // Native <video src> silently fails on them due to CORS / missing proxy headers.
    // We must load them via HLS.js (XHR-based) so the browser can actually fetch them.
    const isGiftedProxy = streamUrl.includes("/api/v2/stream");
    const useHls = (isM3u8 || isGiftedProxy) && Hls.isSupported();
    ...
    if (useHls) {
      ...
      // Native fallback — used for download_url (direct MP4, no proxy headers needed)
      video.src = streamUrl;
  ...
  if (matchingId || loadingSources || (matchEnabled && !subjectId && !sourcesError)) {
    return (
  ...
  if (sourcesError || sources.length === 0) {
    return (
      <div className="fixed inset-0 bg-black flex flex-col items-center justify-center gap-4 z-50 p-6 text-center">
        ...
        <h2 className="text-2xl font-bold text-white">No Sources Found</h2>
        <p className="text-white/60 max-w-md">
          We couldn't find any playable sources for this title. Try again or go back and select something else.
        </p>
