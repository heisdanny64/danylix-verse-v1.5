...
const SUBJECT_CACHE_KEY = "dverse_gifted_subject_cache_v7";
...
export async function findBestMatch(opts: MatchOptions): Promise<string | number | null> {
  ...
  // Use the raw title as the primary query. Only add the normalized form as a
  // second query if it's meaningfully different (not just a case change).
  ...
  // Single-token titles (Invincible, Arcane, etc.) need looser year handling
  // because Gifted sometimes indexes the latest season date, not the premiere date.
  ...
  const scoreCandidate = (r: GiftedSearchItem) => {
    ...
    // Exact match: normalized titles are equal OR the query norm is fully
    // contained within the result title (handles subtitle suffixes like
    // "Arcane" matching "Arcane: League of Legends")
  ...
  // If still no confident match, try page 2 — some titles appear deeper in
  // Gifted's results, especially after newer content pushes them down.
