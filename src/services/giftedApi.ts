...
const SUBJECT_CACHE_KEY = "dverse_gifted_subject_cache_v10";
...
// Matches a season range — e.g. "S1-S4", "S1-S3", "Season 1-3"
const BUNDLE_RX = /\b(s\d+\s*-\s*s?\d+|season\s*\d+\s*-\s*\d+)\b/i;
// Matches a single season label — e.g. "S1", "Season 2"
const SINGLE_SEASON_RX = /\b(s(\d+)|season\s*(\d+))\b/i;

export async function findBestMatch(opts: MatchOptions): Promise<string | number | null> {
  const isTV = opts.type === "tv";
  const seasonNum = isTV && opts.season != null ? opts.season : null;

  // Cache key: include season only for TV when we're looking for individual entries.
  // We don't know yet whether Gifted has bundles or individual entries, so we
  // try the show-level cache first, then fall back to season-specific.
  const showCacheKey = `${opts.type || "x"}:${opts.externalId}`;
  const seasonCacheKey = seasonNum != null ? `${showCacheKey}:s${seasonNum}` : showCacheKey;
  const cache = loadCache<Record<string, string | number | null>>(SUBJECT_CACHE_KEY);

  // Return show-level cache hit (bundle entry) immediately for any season
  if (showCacheKey in cache && cache[showCacheKey] != null) return cache[showCacheKey];
  // Return season-specific cache hit if we have one
  if (seasonNum != null && seasonCacheKey in cache && cache[seasonCacheKey] != null) return cache[seasonCacheKey];
  ...
  // Collect ALL candidates that pass the similarity threshold so we can
  // pick the best bundle vs individual entry after scoring everything.
  const candidates: Array<{ id: string | number; score: number; rTitle: string }> = [];
