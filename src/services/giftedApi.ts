...
const SUBJECT_CACHE_KEY = "dverse_gifted_subject_cache_v8";
...
export interface MatchOptions {
  ...
  season?: number | null; // for TV: prefer the Gifted entry that covers this season
}

export async function findBestMatch(opts: MatchOptions): Promise<string | number | null> {
  // Season-aware cache key: each season of a TV show may map to a different subjectId
  const cacheKey = `${opts.type || "x"}:${opts.externalId}${opts.season != null ? `:s${opts.season}` : ""}`;
  ...
  // Season regex: matches "S1", "Season 1", "S 1" in a result title
  const seasonNum = opts.season != null ? opts.season : null;
  const seasonRx = seasonNum != null
    ? new RegExp(`\\b(s\\s*0*${seasonNum}|season\\s*0*${seasonNum})\\b`, "i")
    : null;
  // Regex that matches ANY season label — used to detect show-level vs season entries
  const anySeasonRx = /\b(s\d+|season\s*\d+)\b/i;
  ...
  const scoreCandidate = (r: GiftedSearchItem) => {
    ...
    // Season-aware scoring for TV shows
    if (seasonNum != null && opts.type === "tv") {
      const hasTargetSeason = seasonRx!.test(rTitle);
      const hasAnySeason = anySeasonRx.test(rTitle);

      if (hasTargetSeason) {
        // Strong bonus: this entry is specifically for the season we want
        score += 0.3;
      } else if (!hasAnySeason) {
        // Mild bonus: show-level entry (no season label) — Gifted serves all
        // seasons from one entry for some shows, so this is a valid fallback
        score += 0.05;
      } else {
        // Has a season label but it's the wrong season — penalize
        score -= 0.35;
      }
    }
